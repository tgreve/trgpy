""" Compilation of local IR, CO, CI, CII and NII data.
This module reads in IR luminosities, CO line fluxes (Jup=1 to Jup=14), and
a number of the main far-IR fine-structure lines.
...
Samples:
	...
	The data were downloaded from the online data servers given below and
	then stored in galaxies_db-samples/
	...
	Jiao+17:
	    (Ho, Omega_L, Omega_m) = (70, 0.7, 0.3)
            Detected lines have positive fluxes and errors.
            Upper limits have negative fluxes, with the abs() value being the 3sigmae upper limit.
...
Comments on extended sources and multiple pointings:
	...
	Rosenberg+15:
		For all extended sources (Arp299, ESO173-G015, MCG+12–02–001, Mrk331, NGC1068
                NGC1365, NGC2146, NGC3256, NGC5135, and NGC7771), an aperture
		correction is necessary to compensate for the wavelength dependent beam size
		(Makiwa et al. 2013). We defined a source as extended using LABOCA or SCUBA 350
		or 450 μm (respectively) maps with 8′′ resolution. We convolved the 8′′
		resolution maps with the SPIRE FTS resolution, and if the galaxy was more
		extended than the smallest SPIRE beam size, we defined it as extended. In order
		to correct for the extended nature of these sources, we employ HIPE’s
		semiExtendedCorrector tool (SECT). This tool “derives” an intrinsic source size
		by iterating over different source sizes until it finds one that provides a
		good match in the overlap range of the two observing bands near 1000 GHz, and
		is further discussed in Wu et al. (2013). We set the Gaussian reference beam to
		42′′, the largest SPIRE beamsize. The beamsize corrected flux values for the 10
		extended sources are listed in Table 2, along with the compact sources


		There are three targets in the sample that have multiple pointings; Arp 299,
		NGC 1365, and NGC 2146. In the case of Arp 299, we use only the pointing for
		Arp 299 A. For NGC 1365 we take the average of the northeast and southwest
		pointings. This is done since the northeast and southwest pointings have
		approximately a 50% overlap in field of view at the center of the galaxy. This
		overlap region is the center of the PACS observations, so for comparison, it is
		best to average the northeast and southwest pointings. For NGC 2146, we use
		only the nuclear pointing.


Run Mongod database:
		>mongod --dbpath=/Users/tgreve/Dropbox/Work/local/python/trgpy/src/local-galaxies-data/db
                >brew services start mongodb
                >mongod --config /usr/local/etc/mongod.conf


STATUS:
    10.12.2018  :   Updated to work with python 3.x
"""
...
# --- Global variables ---
# Path to local data
data_path = '/Users/tgreve/Dropbox/Work/local/python/trgpy/src/galaxies_db-samples/'

# Data table names
table1_name_L17 = "Lu-et-al-2017-table-1.txt"
table4_name_L17 = "Lu-et-al-2017-table-4.txt"

table1_name_K16 = "Kamenetzky-et-al-2015-Table-1.txt"
table2_name_K16 = "Kamenetzky-et-al-2015-Table-2.tsv"
table3_name_K16 = "Kamenetzky-et-al-2015-Table-3.tsv"
...
cosmo_params_R15= {'omega_M_0': 0.28, 'omega_lambda_0': 0.72,
                   'omega_k_0': 0.0, 'h': 0.70, 'Tcmb0': 2.725}
cosmo_params_I15= {'omega_M_0': 0.27, 'omega_lambda_0': 0.73,
                   'omega_k_0': 0.0, 'h': 0.73, 'Tcmb0': 2.725}
cosmo_params_L17  = {'omega_M_0': 0.3, 'omega_lambda_0': 0.7, 'omega_k_0': 0.0,
                     'h': 0.70, 'Tcmb0': 2.73}
...
def build(do_pickle=True, verbose=False, commit_all_to_db=True):
    ...
    # Make master list
    make_master_list(verbose=verbose)
...
def map_id_raw_to_id(table, catalogue=False, drop=True):
    ...
    # --- Format A09 ---
    if catalogue == 'A09':
        # If ALT_ID_RAW is nan, replace with ID (with IRAS added to it)
        i=0
...
def make_pickle_A09():
    ...
    for i in np.arange(0,len(table.ID_RAW)):
        ...
        _progressBar("Armus+09 sample: ", i, len(table.ID_RAW))
    ...
    df = pd.DataFrame({'ID_RAW': table.ID_RAW, 'ID_ALT_RAW': table.ID_ALT_RAW,
                       'ID': table.ID, 'ID_ALT': table.ID_ALT,
                       'z': z, 'LIR_8_1000': table.LIR_8_1000})
...
def make_pickle_R15():
    ...
    # --- Fix LIR and convert into standard cosmology ---
    cosmo_R15 = FlatLambdaCDM(H0=100.*cosmo_params_R15['h'],
            Om0=cosmo_params_R15['omega_M_0'], Tcmb0=cosmo_params_R15['Tcmb0'])
    d_L_R15 = cosmo_R15.luminosity_distance(table1.z)
    d_L_R15 = d_L_R15.value
    cosmo = FlatLambdaCDM(H0=100.*cosmo_params['h'],
            Om0=cosmo_params['omega_M_0'], Tcmb0=cosmo_params['Tcmb0'])
    d_L = cosmo.luminosity_distance(table1.z)
    d_L = d_L.value
    correction_factor = (d_L/d_L_R15)**2.

    table1.loc[:,('LIR_8_1000')] = table1.LIR_8_1000*correction_factor
    # ---------------------------------------------------
    ...
    # --- Pickle data ---
    df = pd.DataFrame({'ID_RAW': table1.ID_RAW, 'ID': table1.ID, 'z': table1.z, 'LIR_8_1000': table1.LIR_8_1000, 'FIR': table1.FIR,
                       'AGN_SB_type': table1.AGN_SB_type, 'SdV_CO10': table3.SdV_CO10, 'SdV_CO21': table3.SdV_CO21,
                      'SdV_CO32': table3.SdV_CO32, 'SdV_CO43': table2.SdV_CO43, 'SdV_CO54': table2.SdV_CO54,
                      'SdV_CO65': table2.SdV_CO65, 'SdV_CO76': table2.SdV_CO76, 'SdV_CO87': table2.SdV_CO87,
                      'SdV_CO98': table2.SdV_CO98, 'SdV_CO109': table2.SdV_CO109, 'SdV_CO1110': table2.SdV_CO1110,
                      'SdV_CO1211': table2.SdV_CO1211, 'SdV_CO1312': table2.SdV_CO1312, 'SdV_CI609': table2.SdV_CI609,
                      'SdV_CI370': table2.SdV_CI370, 'SdV_OI63': table2.SdV_OI63, 'SdV_OI145': table2.SdV_OI145,
                      'SdV_CII158': table2.SdV_CII158})
...
def make_pickle_I15():
    ...
    z=[]
    for l in table1.DL:
        z.append(z_at_value(cosmo_I15.luminosity_distance, l*u.Mpc, zmax=6700))
    z=np.array(z)
    # ---------------------------------

    # --- Fix LIR and convert into standard cosmology ---
    d_L_I15 = cosmo_I15.luminosity_distance(z)
    d_L_I15 = d_L_I15.value
    cosmo = FlatLambdaCDM(H0=100.*cosmo_params['h'],
            Om0=cosmo_params['omega_M_0'], Tcmb0=cosmo_params['Tcmb0'])
...
def make_pickle_K16():
    """ Creates K16 pkl file.
        18.12.18: Finalized. Sets nan entries to ValUnDef.
    """
    ...
    # Calculate LIR_40_120. If NaN set to ValUnDef
    for i in np.arange(0,len(table1.LIR_40_120)):
        if table1.LIR_40_120[i] > 0:
            table1.loc[i,'LIR_40_120'] = 10.**(table1.LIR_40_120[i])
        else:
            table1.loc[i,'LIR_40_120'] = ValUnDef
    ...
    for transition in transitions:
        # Create columns in table1 for each transition SdV, eSdV, SdV_3sigmaUL = ValUnDef
        ...
        for j in range(0, N3):
            ...
            # Flux errors (e_high-e_low)/2. Always exist in table.
            ...
            if len(indices_ID) > 0:
                ...
                if SdV_3sigma_ul > 0:
                    ...
                    table1.loc[indices_ID, ('SdV_3sigmaUL_'+entries[q])] = ValUnDef
            else:
                print("Error...")
        q=q+1
    ...
    # --- Pickle Table 1 ---
    df = pd.DataFrame({'ID_RAW': table1.ID_RAW,'ID': table1.ID, 'z': table1.z, 'LIR_40_120': table1.LIR_40_120,
                       'SdV_CO43': table1.SdV_CO43, 'SdV_CO54': table1.SdV_CO54,
                       'SdV_CO65': table1.SdV_CO65, 'SdV_CO76': table1.SdV_CO76, 'SdV_CO87': table1.SdV_CO87,
                       'SdV_CO98': table1.SdV_CO98, 'SdV_CO109': table1.SdV_CO109, 'SdV_CO1110': table1.SdV_CO1110,
                       'SdV_CO1211': table1.SdV_CO1211, 'SdV_CO1312': table1.SdV_CO1312, 'SdV_CI609': table1.SdV_CI609,
                       'SdV_CI370': table1.SdV_CI370, 'SdV_NII205': table1.SdV_NII205,
                       'eSdV_CO43': table1.eSdV_CO43,
                       'eSdV_CO54': table1.eSdV_CO54, 'eSdV_CO65': table1.eSdV_CO65, 'eSdV_CO76': table1.eSdV_CO76,
                       'eSdV_CO87': table1.eSdV_CO87, 'eSdV_CO98': table1.eSdV_CO98, 'eSdV_CO109': table1.eSdV_CO109,
                       'eSdV_CO1110': table1.eSdV_CO1110, 'eSdV_CO1211': table1.eSdV_CO1211,
                       'eSdV_CO1312': table1.eSdV_CO1312, 'eSdV_CI609': table1.eSdV_CI609, 'eSdV_CI370': table1.eSdV_CI370,
                       'eSdV_NII205': table1.eSdV_NII205,
                       'SdV_3sigmaUL_CO43': table1.SdV_3sigmaUL_CO43, 'SdV_3sigmaUL_CO54': table1.SdV_3sigmaUL_CO54,
                       'SdV_3sigmaUL_CO65': table1.SdV_3sigmaUL_CO65, 'SdV_3sigmaUL_CO76': table1.SdV_3sigmaUL_CO76, 'SdV_3sigmaUL_CO87': table1.SdV_3sigmaUL_CO87,
                       'SdV_3sigmaUL_CO98': table1.SdV_3sigmaUL_CO98, 'SdV_3sigmaUL_CO109': table1.SdV_3sigmaUL_CO109, 'SdV_3sigmaUL_CO1110': table1.SdV_3sigmaUL_CO1110,
                       'SdV_3sigmaUL_CO1211': table1.SdV_3sigmaUL_CO1211, 'SdV_3sigmaUL_CO1312': table1.SdV_3sigmaUL_CO1312, 'SdV_3sigmaUL_CI609': table1.SdV_3sigmaUL_CI609,
                       'SdV_3sigmaUL_CI370': table1.SdV_3sigmaUL_CI370, 'SdV_3sigmaUL_NII205': table1.SdV_3sigmaUL_NII205})
...
def make_pickle_L17():
    ...
    d_L_L17 = cosmo_L17.luminosity_distance(z)
    d_L_L17 = d_L_L17.value
    cosmo = FlatLambdaCDM(H0=100.*cosmo_params['h'],
            Om0=cosmo_params['omega_M_0'], Tcmb0=cosmo_params['Tcmb0'])
    d_L = cosmo.luminosity_distance(z)
    d_L = d_L.value
    correction_factor = (d_L/d_L_L17)**2.

    for i in np.arange(0,len(table1.ID_RAW)):
        if float(table1.logLIR_8_1000.loc[i]) > -99:
    ...
    for k in ['2','3', '4']: #, '5']:
        ...
        for i in np.arange(0,len(df['ID'])):
            ...
            for entry in entries:
                ...
                # ID in db['ID']
                if not(cursor == None):
                    print(ID, ID_ALT, cursor_alt)
                    # SdV is detected (always the case for I15)
                    if SdV > 0:
                        # Commit to db
                        SdV = line_flux_conversion(freq[transitions[j]], SdV, conversion='si2jansky')
                        db.local_galaxies.update_one(
                            {
                                'ID': {
                                    "$eq": ID
                        ...
                        eSdV = line_flux_conversion(freq[transitions[j]], eSdV, conversion='si2jansky')
                        db.local_galaxies.update_one(
                            {
                                'ID': {
                                    "$eq": ID
...
def commit_to_db_K16():
    """ Commits K16 data to db.
        18.12.18: Finalized. Does not include ValUnDef entries in db.
    """
    ...
    # --- Transitions contributed by K16
    transitions = ['4-3','5-4','6-5','7-6','8-7','9-8','10-9','11-10','12-11',
                   '13-12']
    transitions = ['12CO(' + s + ')' for s in transitions]
    transitions = transitions + ['[CI]609','[CI]370','[NII]205']
        ...
        for entry in entries:
            ...
            if not(cursor == None):
                ...
                # Upper limit
                    ...
                    SdV_3sigmaUL = line_flux_conversion(freq[transitions[j]], SdV_3sigmaUL, conversion='si2jansky')
                    # Commit to db
                    db.local_galaxies.update_one(
                        {
                            'ID': {
